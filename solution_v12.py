...
# Пробуем разные длины body
best_map = 0
best_config = None
best_results = None

for max_len in [1500, 2000, 3000, 5000, None]:
    if max_len:
        articles["trunc_body"] = articles["clean_body"].apply(lambda x: x[:max_len])
    else:
        articles["trunc_body"] = articles["clean_body"]
    
    articles["full_text"] = (articles["title_text"] + " ") * 3 + articles["trunc_body"]
    ...
    print("Grid search BM25 params...")
    local_best_map = 0
    local_best_config = None
    
    k1_values = [1.8, 2.0, 2.2]
    b_values = [0.4, 0.5, 0.6]
    weights_options = [
        [1.0, 1.0, 1.0, 1.0, 1.8, 0.8],
        [1.0, 1.0, 0.8, 1.0, 2.0, 0.8],
        [1.0, 1.0, 1.0, 1.2, 1.8, 0.8],
    ]
    ...
    for k1 in k1_values:
        for b in b_values:
            ...
            def get_rankings(query, candidate_k=50):
                ...
                return [top_ids(s) for s in [bm25_scores, bm25_st_scores, bm25_stop_scores, tfidf_scores, emb_scores, title_scores]]
            
            cal_rankings = [get_rankings(q, candidate_k=50) for q in cal_queries]
            
            for weights in weights_options:
                for k_val in [30, 40]:
                    ap_scores = [map_at_k(rrf(r, weights=weights, k=k_val)[:10], rel) for r, rel in zip(cal_rankings, cal_relevant)]
                    map_score = np.mean(ap_scores)
                    if map_score > local_best_map:
                        local_best_map = map_score
                        local_best_config = {"k1": k1, "b": b, "weights": weights, "k": k_val, "max_len": max_len}
    
    print(f"  Best for max_len={max_len}: MAP@10={local_best_map:.4f}, config={local_best_config}")
    
    if local_best_map > best_map:
        best_map = local_best_map
        best_config = local_best_config

print(f"\nOverall Best MAP@10 = {best_map:.4f}")
print(f"Best config = {best_config}")

# Пересоздаём финальные данные с лучшей длиной
max_len_best = best_config["max_len"]
if max_len_best:
    articles["trunc_body"] = articles["clean_body"].apply(lambda x: x[:max_len_best])
else:
    articles["trunc_body"] = articles["clean_body"]
articles["full_text"] = (articles["title_text"] + " ") * 3 + articles["trunc_body"]
articles["tokens"] = articles["full_text"].apply(lambda x: tokenize(x, False))
...
bm25 = BM25Okapi(articles["tokens"].tolist(), k1=best_config["k1"], b=best_config["b"])
bm25_stem = BM25Okapi(articles["tokens_stem"].tolist(), k1=best_config["k1"], b=best_config["b"])
bm25_stop = BM25Okapi(articles["tokens_stop"].tolist(), k1=best_config["k1"], b=best_config["b"])
tfidf = TfidfVectorizer(ngram_range=(1, 2), max_df=0.95, min_df=1)
tfidf_matrix = tfidf.fit_transform(articles["full_text"].tolist())
...
def rank_query(query, top_k=10, candidate_k=50):
    ...
    return rrf(rankings, weights=best_config["weights"], k=best_config["k"])[:top_k]

print("\nValidating final on calibration...")
