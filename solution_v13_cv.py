...
def build_features_for_candidates(query, candidate_ids):
    ...
    for aid in candidate_ids:
        ...
        for key in sorted(scores.keys()):
            f.extend([scores[key][idx], normalized[key][idx], ranks[key][idx]])
        f.extend([len(query), len(articles.iloc[idx]["full_text"]), len(articles.iloc[idx]["title_text"])])
        features.append(f)
...
print("Precomputing all candidate features...")
...
print(f"Total samples: {X_all.shape[0]}, positives: {y_all.sum()}")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(all_queries)):
    ...
    y_train = np.array([all_labels[start:start+50] for i in train_idx 
                        for start in [sum(len(all_candidates[j]) for j in range(i))]]).flatten()
    
    # Правильная индексация labels
