...
article_embeddings = model.encode(article_texts, batch_size=32, show_progress_bar=True, normalize_embeddings=True)
...
def get_hybrid_ranking(query, top_k=10):
    ...
    tfidf_ranking = get_tfidf_ranking(query, top_k=50)
    emb_ranking = get_embedding_ranking(query, top_k=50)
    fused = reciprocal_rank_fusion(
        [bm25_ranking, tfidf_ranking, emb_ranking],
        weights=[1.0, 1.0, 1.5],
        k=60
    )
    return fused[:top_k]
