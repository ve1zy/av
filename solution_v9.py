...
for key, name in model_names.items():
    ...
    full_emb = encoder.encode(articles["full_text"].tolist(), batch_size=128, show_progress_bar=False, normalize_embeddings=True)
    title_emb = encoder.encode(articles["title_text"].tolist(), batch_size=128, show_progress_bar=False, normalize_embeddings=True)
    embeddings[key] = (encoder, full_emb, title_emb)
...
def get_rankings(query, candidate_k=50):
    ...
    rankings = [bm25_scores, bm25_st_scores, bm25_stop_scores, tfidf_scores]
    labels = ["bm25", "bm25_stem", "bm25_stop", "tfidf"]
    
    for key, (encoder, full_emb, title_emb) in embeddings.items():
        q_emb = encoder.encode([query], show_progress_bar=False, normalize_embeddings=True)[0]
        rankings.append(full_emb @ q_emb)
        labels.append(f"emb_{key}")
        rankings.append(title_emb @ q_emb)
        labels.append(f"title_emb_{key}")
    
    def top_ids(scores):
        return [article_ids[i] for i in np.argsort(scores)[::-1][:candidate_k]]
    
    return [top_ids(s) for s in rankings]

print("Computing calibration rankings...")
cal_rankings = [get_rankings(q, candidate_k=50) for q in cal_queries]

print("Running grid search (coarse)...")
best_map = 0
best_weights = None
best_k = None

# Базовые веса + variation
base_options = [
    [1.0, 1.0, 0.8, 1.0, 1.8, 0.8, 1.5, 0.5, 1.0, 0.3],
    [1.0, 1.0, 0.8, 1.0, 2.0, 1.0, 1.2, 0.5, 1.0, 0.3],
    [1.0, 1.0, 0.5, 1.0, 2.2, 1.0, 1.0, 0.8, 0.8, 0.5],
    [1.0, 0.8, 0.8, 1.2, 2.0, 0.8, 1.5, 0.5, 1.0, 0.3],
    [1.0, 1.0, 0.8, 0.8, 2.5, 1.0, 1.0, 0.3, 1.0, 0.3],
]

for k_val in [30, 40, 60]:
    for weights in base_options:
        ap_scores = []
